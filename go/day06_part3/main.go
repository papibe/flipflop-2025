...
func solve(speeds []Speed) int {
	...
	for picture := 1; picture <= 1000; picture++ {
		...
		for _, speed := range speeds {
			x_final := mod(speed.x_step*31556926*picture, 1000)
			y_final := mod(speed.y_step*31556926*picture, 1000)

			if 250 <= x_final && x_final <= 749 && 250 <= y_final && y_final <= 749 {
				n_birds++
			}
		}
...
func main() {
	fmt.Println(solution("./input.txt")) // 247208
}
